function [Keypoints1, Keypoints2] = FindTheSparseAudioClipsKeypoints_(sparseCoefficientMatrix, varargin)
%
%   Input:: 
...
%       Keypoints2 
    ...
    for i=1:size(sparseCoefficientMatrix,2)
        ...
        [sortedCoefficients_i, indices_i]= sort(sparseCoefficientMatrix_i,'descend'); 
        
        k=0; 
        for j=1:length(sortedCoefficients_i)
            p_k = sortedCoefficients_i(j); 
            l_k = indices_i(j); 
            
            if k<numberOfKeypointsPerFrame1 
                if sparseCoefficientMatrix_i(l_k)> threshold1
                    k=k+1; 
