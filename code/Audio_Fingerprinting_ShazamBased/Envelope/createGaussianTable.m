function [G, gaussian_table]  = createGaussianTable(X,sigma)
...
%   Output:: 
    ...
    %   compute the local maximum points of the input matrix 
    rho = getLocalMaxima(X); 
    
    %   create the gaussian kernel 
    for f=1:length(X)
        
        for m=1:length(rho)
            G(f,m)= exp( -0.5*( ( (f-m)/sigma)^2)); 
        end 
    end 
